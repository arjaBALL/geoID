...
export default function GeoLocation() {
  ...
  const fetchEmployees = async () => {
    try {
      ...
      const mapped = (data ?? []).map((u) => ({
        ...
        role: u.roles?.name ?? "Unassigned",
        section: u.sections?.section_name ?? null,
      }));
  ...
  const fetchGeofences = async () => {
    try {
      ...
      const mapped = (data ?? []).map((g) => ({
        ...
        employeeIds: (g.geofence_employees ?? []).map((e) => String(e.user_id)),
      }));

      setSavedGeofences(mapped);
    } catch (err) {
      Alert.alert("Error", err?.message ?? "Failed to load geofences.");
    }
  };

  const confirmSave = async () => {
    const name = nameInput.trim() || `Geofence ${savedGeofences.length + 1}`;
    const computedArea = polygonAreaMeters(points);
    ...
    try {
      const { data, error } = await supabase
        ...
        .insert({
          name,
          points, // jsonb column, array of {latitude, longitude} is fine
      ...
      setTimeout(() => openEmployeeModal(newEntry), 250);
    } catch (err) {
  ...
  const confirmEmployeeAssignment = async () => {
    try {
      // Replace all assignments for this geofence
      const { error: deleteError } = await supabase
        .from("geofence_employees")
        .delete()
        .eq("geofence_id", employeeModalGeofenceId);

      if (deleteError) throw deleteError;

      if (selectedEmployeeIds.length > 0) {
        const rows = selectedEmployeeIds.map((userId) => ({
          geofence_id: employeeModalGeofenceId,
          user_id: userId,
        }));
      ...
      setSavedGeofences((prev) =>
        prev.map((g) =>
          g.id === employeeModalGeofenceId
            ? { ...g, employeeIds: selectedEmployeeIds }
            : g,
        ),
      );
      ...
      Alert.alert(
        "Error",
        err?.message ?? "Failed to save employee assignment.",
      );
    }
  };
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        moveCamera(currentCoordinates, 15);
      } catch (err) {
  ...
  const moveCamera = useCallback((coordinates, zoom = 15) => {
    if (!mapRef.current) return;

    try {
      const result = mapRef.current.setCameraPosition({
        coordinates,
        zoom,
        duration: 300,
      });
      ...
      if (result?.catch) {
        result.catch((error) => {
          ...
          if (!message.includes("animation cancelled")) {
        ...
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert(
                "Error",
                err?.message ?? "Failed to delete geofence.",
              );
            }
          },
        },
  ...
  return (
    <View className="flex-1 bg-[#0F1620] mt-10">
      {/* =================================================
          MAP
      ================================================= */}
              ...
              ListEmptyComponent={
                <View className="items-center py-10">
                  <View className="h-12 w-12 items-center justify-center rounded-2xl bg-white/5">
                    <FontAwesome6
                      name={
                        employeesError ? "triangle-exclamation" : "user-slash"
                      }
                      size={16}
                      color="#4B5563"
                      iconStyle="solid"
